...
function Home() {
  ...
  const fetchAllRecipes = async () => {
    try {
      const response = await fetch("https://recipehub-backend-cx3k.onrender.com/api/recipes");
      if (!response.ok) {
...
const styles = {
  ...
  slider: {
    ...
    justifyContent: "center",
    alignItems: "center",
    boxShadow: "0px 11px 115px rgb(255, 119, 0)", // Added shadow effect
  },
  slideImage: {
    position: "absolute",
    width: "100%",
  ...
  title: {
    fontSize: "50px",
    fontWeight: "300", // Made the title thinner
    marginBottom: "10px",
    zIndex: 2,
  },
  subtitle: {
  ...
  exploreBtn: {
    padding: "10px 20px",
    fontSize: "18px",
    background: "#ff7500", // Applied the requested color
    color: "#fff",
    border: "none",
    cursor: "pointer",
    borderRadius: "5px",
